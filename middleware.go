...
const (
	stackInherit  stackMode = iota // use the app default stack unchanged
	stackExtend                    // app default stack, then these
	stackOverride                  // exactly these, ignoring the default stack
)

// MiddlewareStack is an [Endpoint]'s middleware declaration, relative to the
...
func ExtendMiddleware(m ...Middleware) MiddlewareStack {
	return MiddlewareStack{mode: stackExtend, stack: m}
}
...
func OverrideMiddleware(m ...Middleware) MiddlewareStack {
	return MiddlewareStack{mode: stackOverride, stack: m}
}
...
func NoMiddleware() MiddlewareStack {
...
func (s MiddlewareStack) resolve(defaults []Middleware) []Middleware {
	switch s.mode {
	case stackOverride:
		return s.stack
	case stackExtend:
